...
function ResultPage(){
    ...
    allAnswers.updatePersonalData(1,69,180,170,110,1,3.5,0,1,0);
    allAnswers.updateFamilyData(1,0);
    allAnswers.updateHabitsData(1,2,3,2)
    ...
    const changeCancer = (event) => { setCancer(myAlgo.Cancer(allAnswers.afCancer,smokeValue,allAnswers.bmiAlgo,sportValue,alcoolValue,allAnswers.alimAlgo))}
    const changeDiabete = (event) => {setDiabete(myAlgo.Diabete(allAnswers.sex,allAnswers.age,allAnswers.bmiAlgo,allAnswers.systAlgo,allAnswers.glycAlgo,sportValue,alimValue))}
    const changeInfarctus = (event) => {setInfarctus(myAlgo.Infarctus(allAnswers.age,allAnswers.sex,smokeValue,allAnswers.systAlgo,allAnswers.diab,allAnswers.inf,allAnswers.cholAlgo,allAnswers.hdlAlgo))}
    const changeNonInfarctus = (event) => {setNonInfarctus(myAlgo.NonInfractus(allAnswers.age,smokeValue,allAnswers.systAlgo,allAnswers.cholAlgo,allAnswers.hdlAlgo,allAnswers.afinf,allAnswers.sex))}
                ...
                <div className="column">
                    ...
                    <div className="allColumnText">
                        <div className="textAndSlider">
                           ...
                           <input type="range" min="0" max="1" onChange={changeSmoke} value={smokeValue} step={1} className="slider"/>
                        </div>
                        <div className="textAndSlider">
                            <p className="child" style={{alignSelf: "start"}}>Alim value {alimValue}</p>
                            <input type="range" min="0" max="4" onChange={changeAlim} value={alimValue} step={1} className="slider"/>
                        </div>
                        <div className="textAndSlider">
                            <p className="child" style={{alignSelf: "start"}}>Sport value {sportValue}</p>
                            <input type="range" min="0" max="4" onChange={changeSport} value={sportValue} step={1} className="slider"/>
                        </div>
                        <div className="textAndSlider">
                            <p className="child" style={{alignSelf: "start"}}>weight value {weightValue}</p>
                            <input type="range" min="0" max="200" onChange={changeWeight} value={weightValue} step={1} className="slider"/>
                        </div>
                        <div className="textAndSlider">
                            <p className="child" style={{alignSelf: "start"}}>alcool value {alcoolValue}</p>
                            <input type="range" min="0" max="4" onChange={changeAlcool} value={alcoolValue} step={1} className="slider"/>
                        </div>
                    </div>
                </div>
